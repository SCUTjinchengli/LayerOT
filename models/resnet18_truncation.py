...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, truncation, num_classes=1000, zero_init_residual=False,
        ...
        self.layer2_0 = self._make_layer(block, 128, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer2_1 = self._make_layer(block, 128, layers[3], stride=1,
                                       dilate=replace_stride_with_dilation[0])

        self.layer3_0 = self._make_layer(block, 256, layers[4], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.layer3_1 = self._make_layer(block, 256, layers[5], stride=1,
                                       dilate=replace_stride_with_dilation[1])

        self.layer4_0 = self._make_layer(block, 512, layers[6], stride=2,
                                       dilate=replace_stride_with_dilation[2])
        self.layer4_1 = self._make_layer(block, 512, layers[7], stride=1,
                                       dilate=replace_stride_with_dilation[2])     
...
def resnet18_trunc(pretrained=False, progress=True, truncation=None, num_classes=1000, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet18', BasicBlock, [1, 1, 1, 1, 1, 1, 1, 1], pretrained, progress, truncation, num_classes,
                   **kwargs)